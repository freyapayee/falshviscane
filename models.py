...
class AgronomicLog(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='CASCADE'), nullable=False)
    variety = db.Column(db.String(120), nullable=True)
    hectares = db.Column(db.String(50), nullable=True)
    plowing_count = db.Column(db.String(20), nullable=True)
    weeding_count = db.Column(db.String(20), nullable=True)
    fertilizer_count = db.Column(db.String(20), nullable=True)
    ratoon_stage = db.Column(db.String(40), nullable=True)
    rssi_infected = db.Column(db.String(20), nullable=True)
    predicted_lkg_tc = db.Column(db.Float, nullable=True)
    predicted_tc_ha = db.Column(db.Float, nullable=True)
    predicted_lkg = db.Column(db.Float, nullable=True)
    recommendations_summary = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
